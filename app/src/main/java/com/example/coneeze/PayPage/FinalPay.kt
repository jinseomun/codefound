...
fun FinalPay(price: String) {
    Column(
        ...
        Row(
            ...
            Text(
                text = price,  // 여기에서 전달받은 price를 사용
                style = TextStyle(
                    fontSize = 16.sp,
                    lineHeight = 20.sp,
                    fontFamily = suit,
        ...
        FinalPayDetail(text = "총 상품 금액", money = price) // 전달받은 price 사용
        Spacer(modifier = Modifier.height(8.dp))

        FinalPayDetail(text = "배송비", money = "0원")
        Spacer(modifier = Modifier.height(8.dp))

        FinalPayDetail(text = "할인 쿠폰", money = "-0원")
        ...
        Divider(
            ...
            modifier = Modifier
                .fillMaxWidth()
                .padding(horizontal = 20.dp) // 필요에 따라 좌우 여백 설정
        )
